...
from core.api.schemas.exams import (
    ExamResponse,
    ExamCreate,
    ExamResponseWithEmployee,
    ExamCreated,
    ExamUpdate,
)
...
async def get_all_exams_by_id(
    session: AsyncSession, skip: int | None, limit: int | None, id: int
) -> list[ExamResponse]:
    stmt = (
        select(Exam)
        .options(joinedload(Exam.exam_type))
        .where(Exam.employee_id == id)
        .order_by(Exam.next_date.desc())
        .offset(skip)
        .limit(limit)
    )
...
async def get_all_exams_from_db(
    skip: int, limit: int, session: AsyncSession
) -> list[ExamResponseWithEmployee]:
    stmt = (
        select(Exam)
        .options(joinedload(Exam.exam_type), joinedload(Exam.employee))
...
async def update_exam_in_db(
    session: AsyncSession, exam_id: int, exam_update: ExamUpdate
):
    try:
        ...
        await session.commit()
        stmt = (
            select(Exam).options(joinedload(Exam.exam_type)).where(Exam.id == exam_id)
        )
        res: Result = await session.execute(stmt)
...
async def get_exams_for_next_month_from_db(session: AsyncSession):
    ...
    stmt = (
        select(Exam)
        .options(joinedload(Exam.exam_type), joinedload(Exam.employee))
        .where(and_(Exam.next_date <= last_day, Exam.next_date >= first_day))
    )
    res: Result = await session.execute(stmt)
    exams: list[Exam] = res.scalars().all()
    return exams
