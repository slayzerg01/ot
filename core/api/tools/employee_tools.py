...
async def get_all_employees_from_db(
    session: AsyncSession, skip: int, limit: int, subdivision: str | None
) -> list[EmployeeSchema]:
    stmt = (
        select(Employee)
        .options(joinedload(Employee.subdivision), joinedload(Employee.position))
        .offset(skip)
        .limit(limit)
    )
...
async def get_all_employees_from_db_v2(
    session: AsyncSession, skip: int, limit: int, subdivision: str | None
) -> list[EmployeeSchema_v2]:
    stmt = (
        select(Employee)
        .options(joinedload(Employee.subdivision), joinedload(Employee.position))
        .offset(skip)
...
async def get_all_employees_with_exams_from_db(
    ...
    if query:
        stmt = (
            ...
            .filter(Employee.FIO.ilike(f"%{query}%"))
    ...
    for item in employees:
        ...
        if cer:
            ...
            certificate = None
        exams_by_id: list[ExamResponse] = await get_all_exams_by_id(
            session=session, skip=0, limit=10, id=employee.id
        )
        if div.id == division or division == None:
            ...
            result.append(res)
    result.sort(
        key=lambda x: x.exams[0].next_date - datetime.date.today()
        if x.exams
        else datetime.timedelta(days=100000),
        reverse=False,
    )
    return result


async def get_employee_from_db(
    session: AsyncSession, name: str | None, id: int | None
) -> EmployeeSchema:
    stmt = (
        select(Employee)
        .options(joinedload(Employee.subdivision), joinedload(Employee.position))
        .where(or_(Employee.FIO == name, Employee.id == id))
    )
    res: Result = await session.execute(stmt)
...
async def add_employee_in_db(session: AsyncSession, new_employee: CreateEmployee):
    ...
    if certificate:
        ...
        stmt = (
            update(Certificate)
            .where(Certificate.number == new_employee.certificate)
    ...
    try:
        ...
        if employee_update.certificate_id is not None:
            ...
            stmt = select(Certificate).where(
                Certificate.number == employee_update.certificate_id
            )
            ...
            if res.employee_id:
                ...
                stmt = (
                    update(Certificate)
                    .where(Certificate.number == employee_update.certificate_id)
                    .values(employee_id=employee.id)
                )
                await session.execute(stmt)
        id = employee.id
        ...
        stmt = (
            ...
            .options(joinedload(Employee.position), joinedload(Employee.subdivision))
        )
        res: Result = await session.execute(stmt)
        employee: Employee = res.scalar_one_or_none()
        stmt = select(Division).where(Division.id == employee.subdivision.division_id)
...
async def add_import_employee(
    fio: str, position_id: int, subdivision_id: int, session: AsyncSession
):
    employee = Employee()
    employee.FIO = fio
