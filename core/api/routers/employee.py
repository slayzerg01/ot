...
async def read_employees_with_exams(division: int = None, 
                                    query: str = None, 
                                    skip: int = 0, 
                                    limit: int = 100, 
...
async def read_employees(subdivision: str = None, 
                         skip: int = 0, 
                         limit: int = 100, 
                         user: User = Depends(current_active_verified_user),
                         session: AsyncSession = Depends(get_async_session)
...
async def add_employee(new_employee: CreateEmployee, 
                       user: User = Depends(current_active_verified_user),
                       session: AsyncSession = Depends(get_async_session)):
    """
...
async def del_employee(employee: Employee = Depends(employee_by_id), 
                       user: User = Depends(current_active_verified_user),
                       session: AsyncSession = Depends(get_async_session)):
    """
    Delete employee:
...
async def get_employees(employee: Employee = Depends(employee_by_id),
                        user: User = Depends(current_active_verified_user)
                        ) -> EmployeeSchema:
    """
    Get employee:
