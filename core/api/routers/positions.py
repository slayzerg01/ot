...
from core.api.tools.position_tools import (
    get_all_positions_from_db,
...
async def update_position(
    position_update: CreatePosition,
    position: Position = Depends(position_by_id),
    user: User = Depends(current_active_verified_user),
    session: AsyncSession = Depends(get_async_session),
):
    return await update_position_in_db(
        session=session, position=position, position_update=position_update
    )


@router.post("/add")
async def add_position(
    new_position: CreatePosition,
    user: User = Depends(current_active_verified_user),
    session: AsyncSession = Depends(get_async_session),
):
