...
from core.api.tools.subdivision_tools import (
    get_all_subdivisions_from_db,
    add_subdivision,
    get_subdivision_by_id_from_db,
)
    ...
    try:
        ...
        while sheet[f"A{i}"].value:
            ...
            employee_list.append(
                EmployeeInfo(fio=fio, position=position, subdivision=subdivision)
            )
            i += 1
        if i == 2:
            raise CustomException("Excel файл пустой")
        subdivisions: list[SubdivisionBase] = await get_all_subdivisions_from_db(
            session=session
        )
        ...
        for employee in employee_list:
            ...
            if not employee.position[1]:
                new_position = await add_position_in_db(
                    new_position=CreatePosition(name=employee.position[0]),
                    session=session,
                )
                positions.append(
                    PositionRespone(name=new_position.name, id=new_position.id)
                )
                employee.position[1] = new_position.id

            employee_tmp = await get_employee_from_db(
                session=session, name=employee.fio, id=None
            )
            if not employee_tmp:
                await add_import_employee(
                    fio=employee.fio,
...
async def download_file(
    ...
    try:
        ...
        for division in divisions:
            ...
            i += 1
            for exam in exams:
                res: Employee = await get_employee_from_db(
                    session=session, name=None, id=exam.employee_id
                )
                res: Subdivision = await get_subdivision_by_id_from_db(
                    subdivision_id=res.subdivision_id, session=session
                )
        ...
        wb.save(file_path)
        return FileResponse(
            path=file_path, filename=file_name, media_type="multipart/form-data"
        )
    except:
        type, value, traceback = sys.exc_info()
        raise HTTPException(status_code=400, detail=str(value))
